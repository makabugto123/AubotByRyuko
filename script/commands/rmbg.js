...
module.exports.run = async function({ api, event, args }) {
  ...
  try {
    ...
    const response = await axios.get(`https://kaiz-apis.gleeze.com/api/removebg?url=${encodeURIComponent(photoUrl)}`);
    //const processedImageURL = response.data.imageUrl;

    const img = (await axios.get(response, { responseType: "arraybuffer" })).data;
